...
use bevy::{core::{FixedTimestep, Time}, math::Vec3, prelude::{Commands, Entity, IntoSystem, Plugin, Query, Res, ResMut, SpriteBundle, SystemSet, Transform, With}};
...
impl Plugin for EnemyPlugin{
    fn build(&self, app: &mut bevy::prelude::AppBuilder) {
        app.insert_resource(FormationMaker::default()).add_system(enemy_laser_movement.system())
...
fn enemy_spawn(
    ...
    if active_enemies.0 < MAX_ENEMIES {
        ...
        .insert(Enemy).insert(Speed::default()).insert(formation);
...
fn enemy_movement(
    ...
    for (mut tf, speed,mut formation) in query.iter_mut() {
        ...
        // calculate the final x/y (make sure not to overshoot)
